...
export function TransitionModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "The status update could not be saved. Try again."
      );
    }
  };
  ...
  const statusTitles: Record<MistakeStatus, { title: string; desc: string }> = {
    ...
    verified: {
      ...
      desc: "Confirm the discrepancy is genuine and requires vendor recovery.",
    },
    resolved: { title: "Resolve Financial Leakage", desc: "Mark this discrepancy as recovered or settled." },
    dismissed: {
      title: "Dismiss Discrepancy",
      desc: "Mark this finding as false positive or acceptable variance.",
    },
  };
      ...
      <div className="space-y-4 pt-2">
        {requiresReason && (
          <div className="space-y-1.5">
            <label htmlFor="transition-reason" className="block text-xs font-semibold text-foreground">
