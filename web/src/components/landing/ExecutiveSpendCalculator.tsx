...
export function ExecutiveSpendCalculator() {
  ...
  // Leakage model: 1.8% of spend, split across three detection vectors.
  // Parts are rounded first; the total is their exact sum so the math always reconciles.
  const rateLeakageLakhs = Math.round(spendCr * 0.018 * 0.45 * 100);
  const shortShipmentLakhs = Math.round(spendCr * 0.018 * 0.35 * 100);
  const penaltyLakhs = Math.round(spendCr * 0.018 * 0.2 * 100);
  const totalLakhs =
    rateLeakageLakhs + shortShipmentLakhs + penaltyLakhs;

  const fillPct = Math.min(
    100,
    Math.max(0, ((spendCr - MIN_SPEND) / (MAX_SPEND - MIN_SPEND)) * 100)
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="max-w-2xl space-y-3">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Industrial manufacturers and multi-hub distributors average 1.8% recoverable leakage
            across direct materials — split between rate escalations (45%), short deliveries (35%),
            and unapplied SLA penalties (20%).
          </p>
        </div>

        <div className="border border-border bg-card rounded-xl overflow-hidden shadow-sm hover:border-foreground/20 transition-all duration-300">
            ...
            <div className="space-y-3 pt-2">
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full h-2 rounded-lg appearance-none cursor-pointer text-foreground accent-foreground"
                  aria-label="Annual Procurement Spend Slider"
                  aria-valuetext={`₹${spendCr} crores per year`}
                />
              </div>
          ...
          <div className="grid md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-border/50">
            ...
            <div className="p-6 md:p-8 space-y-1 bg-muted/20">
              ...
              <div className="font-mono tabular-nums text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                <NumberTicker value={totalLakhs / 100} decimals={2} prefix="₹" suffix=" Cr" durationMs={450} />
              </div>
              <div className="text-[11px] font-mono text-muted-foreground tabular-nums">
                = {rateLeakageLakhs.toLocaleString("en-IN")} + {shortShipmentLakhs.toLocaleString("en-IN")} +{" "}
                {penaltyLakhs.toLocaleString("en-IN")} L
              </div>
            </div>
            ...
            <div className="p-6 md:p-8 space-y-1">
              ...
              <div className="font-mono tabular-nums text-lg font-semibold text-foreground">
                <NumberTicker value={rateLeakageLakhs} prefix="₹" suffix=" L" durationMs={450} />
              </div>
            ...
            <div className="p-6 md:p-8 space-y-1">
              ...
              <div className="font-mono tabular-nums text-lg font-semibold text-foreground">
                <NumberTicker value={shortShipmentLakhs} prefix="₹" suffix=" L" durationMs={450} />
              </div>
            ...
            <div className="p-6 md:p-8 space-y-1">
              ...
              <div className="font-mono tabular-nums text-lg font-semibold text-foreground">
                <NumberTicker value={penaltyLakhs} prefix="₹" suffix=" L" durationMs={450} />
              </div>
          ...
          <div className="p-4 md:p-6 bg-muted/10 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-xs text-muted-foreground text-center sm:text-left">
              Median recovery across FY24 pilot audits of mid-market manufacturers.
            </span>
            <Button
              href="/signup"
              size="sm"
